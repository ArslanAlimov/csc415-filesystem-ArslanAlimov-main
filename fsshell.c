...
{
#if (CMDMV_ON == 1)
	// Same thing as copy (given by proffessor) but with some minor differences.
	int src_fd;
	int dest_fd;
	char *src;
	char *dest;
	...
	src_fd = b_open(src, O_RDONLY);
	dest_fd = b_open(dest, O_WRONLY | O_CREAT | O_TRUNC);

	do
	{
		readcnt = b_read(src_fd, buf, BUFFERLEN);
		b_write(dest_fd, buf, readcnt);
	} while (readcnt == BUFFERLEN);
...
int main(int argc, char *argv[])
{
	...
	retVal = initFileSystem(volumeSize, blockSize);

	currentDirectory = (fdDir *)malloc(getDirectorySize() * VCBStructure->blockBytes);

	workingDirectory = (char *)malloc(DIRMAX_LEN);

	tempPath = (char *)malloc(DIRMAX_LEN);
	...
	fdDir *directoryArray = (fdDir *)malloc(getDirectorySize() * VCBStructure->blockBytes);

	LBAread(directoryArray, getDirectorySize(), VCBStructure->rootDirLBA);

	memcpy(currentDirectory, directoryArray, getDirectorySize() * VCBStructure->blockBytes);
